...
maybe("PgStore (requires DATABASE_URL)", () => {
  it("round-trips projects, commits, sessions, events, artifacts", async () => {
    ...
    await store.createProject({ id, one_liner: "x", owner_id: "owner-a", origin: "user", phase: "drafting", latest_version: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
    ...
    expect((await store.getProject(id))?.origin).toBe("user");
    expect((await store.listProjects("owner-a")).some((p) => p.id === id)).toBe(true);
    expect((await store.listProjects("owner-b")).some((p) => p.id === id)).toBe(false);
