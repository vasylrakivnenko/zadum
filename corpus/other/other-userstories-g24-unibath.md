...
As a data reuser, I want to access the system in my native language, so that I am not put off reusing University of the product data by language barriers.
...
As an externalcoordinator, I want to have guarantees that my IP rights will not be breached, so that the risk of collaborating with the product is acceptable to me.
AS an externalcoordinator, I want to access data from the product collaborators off campus, so that I can collaborate effectively.
...
As the product Data Archive administrator, I want to make some checks on deposited datasets before they are made public, so that consistent quality of metadata is maintained, compliance with policies can be checked and details of licensing can be checked.
As the product Data Archive administrator, I want to require a minimum set of metadata, so that consistent quality of metadata is maintained.
As the product Data Archive administrator, I want to approve scheduled disposal of data, so that data which is still required is not destroyed.
As the product Data Archive administrator, I want to query the entire archive , so that I can report on particular aspects of the archive holdings.
As the product Data Archive administrator, I want to import the product data from an external data centre wholesale, so that the product data holdings in external archives are not lost if they close down.
As the product Data Archive administrator, I want to encourage and promote the use of open standards for deposit, so that data is as reusable as possible.
