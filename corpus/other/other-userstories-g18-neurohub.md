As a system administrator, I want to run a script that installs the product node onto a virgin Ubuntu operating system.
As a system administrator, I want to run a script that tests to see if the product installation is functioning correctly.
As a release engineer, I want to call a script to execute acceptance tests against the product installation and report back the results.
As an investigator, I want to view the acceptance test results, so that I can understand how feature-complete the product node currently is.
...
As a systems administrator, I want  to install the product dependencies into an Ubuntu 10.04 LTS machine using the normal Ubuntu package management tools.
...
As a researcher, I want to access files stored in my the product workspace, using a network drive attached to my workstation.
As a researcher, I want to create a log book page.
As a release engineer, I want to configure nightly builds that automatically perform the product installation on a virgin operating system, execute tests and report back results, without requiring any manual input/interaction.
...
As a system administrator, I want to migrate data from an older version of the product to the current version.
...
As a user, I want to have a spreadsheet hosted within the product Web page, so that I can edit and calculate costs collaboratively with others while working on the proposal.
As a supervisor, I want to create feedback forms within the product, that my students can fill in and submit, in order to provide me with feedback about the course that they attended.
As a user, I want to have the option to tell the product to push posted content to my Twitter feed at the same time.
As a user, I want to either keep a logbook entry private or share it with individuals rather than groups.
As a user, I want to have the details of the workflow execution to be recorded in the product log book while executing a workflow.
As a user, I want to have that my Twitter Tweets to appear in the product, so that other researchers in the centre can see what I am up to.
As a user, I want to view the group's shared calendars via the product Nodes' Web page as well as on my mobile device.
As a user, I want to sync events created in the product with a web-based Calendar such as Google Calendar.
As a researcher, I want to have the ability to insert Greek symbols into my logbook entries.
As a user, I want to have the ability to move multiple files around and rearrange them using the product file browser while processing data.
...
As a researcher, I want to secure remote access to the product node when working away from the laboratory
As a researcher, I want to have a group-level repository for sharing papers/reviews/comments with other researchers.
As a researcher, I want to have the details of the workflow plan as well as the execution history and results recorded in the product while executing a workflow