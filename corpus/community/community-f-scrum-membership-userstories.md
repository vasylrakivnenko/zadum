...
As a site administrator, I want to create an Other Course or Event that is not charged a listing fee, so that the product doesn't charge itself for Scrum Gatherings that it puts on.
...
As a trainer, I want to load an Excel file into the site, so that the course participants are added to the product records.
...
As a CSM, I want to be sent an email welcoming me to the product and with instructions to register or activate my membership once my name has been loaded to the registry.
...
As a company, I want to join the product by paying a corporate membership fee, so that I can show the company's support of Scrum.
...
As a trainer, I want to read information of relevance only to trainers, so that the product can share information with me easily.
